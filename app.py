...
def register():
    if request.method == 'POST':
        ...
        if user_details['password'] != user_details['confirmPassword']:
            flash('Password do not match! Tru again!', 'danger')
    ...
    if request.method == 'POST':
        ...
        if result_value > 0:
            ...
            if check_password_hash(user['password'], user_details['password']):
                ...
                flash('Welcome ' + session['first_name'] + '! You have been successfuly logged in!', 'success')
            else:
                cursor.close()
...
def write_blog():
    if request.method == 'POST':
        ...
        mysql.connection.commit()
        cursor.close()
        flash('You blogpost is successfully posted!', 'success')
        return redirect('/')
...
def delete_blog(id):
    ...
    mysql.connection.commit()
    flash("Your blog hash been deleted!", "success")
